...
def run_models(site_colname, csv_path, y_var,nemo_path, yvar_colname,subid_colname,chronicity_colname,subsets,nemo_settings, models_tested, verbose, covariates, lesionload_types, nperms, save_models, ensembles,hcp_dir, atlases, chaco_types, crossval_types, null, results_path, output_path, figs_only, analysis_id, workbench_vis,scenesdir, wbpath,boxplots, override_rerunmodels, ensemble_atlas,final_model):
                        ...
                        for crossval in crossval_types:
                            ...
                            for model_tested in models_tested_chaco:
                                for subset in subsets:
                                    ...
                                    [X, Y, C, lesion_load, site, chronic_ids, ignore2, ignore] = create_data_set(csv_path,site_colname,nemo_path,yvar_colname,subid_colname,chronicity_colname,atlas,covariates, verbose, y_var, chaco_type, subset,1,nemo_settings=nemo_settings,ll= lesionload_type)
                                    ...
                                    if subset == 'acutechronic':
                                        ...
                                        [acuteX, acuteY,acuteC, acute_lesion_load, acute_site,acute_ids,ignore2,ignore] = create_data_set(csv_path,site_colname,nemo_path,yvar_colname, subid_colname,chronicity_colname,atlas,covariates, verbose, y_var, chaco_type,'acute',1,nemo_settings,ll= lesionload_type)

                                        acute_data = {'acute_X':acuteX,'acute_Y':acuteY, 'acute_LL':acute_lesion_load, 'acute_C':acuteC, 'acute_site':acute_site}
                                    else:
                                        acute_data = []
                                    ...
                                    if not override_rerunmodels: # if we don't want to override model results
                                        ...
                                        if files_exist: # we dont want to override but dont recalculate what's already been done
                                            ...
                                            if figs_only: 
                                                ...
                                                set_up_and_run_model(crossval, model_tested,lesion_load, lesionload_type, X, Y, C, site, atlas, y_var, chaco_type, subset, save_models, results_path, nperms, null,ensemble, output_path,ensemble_atlas,chaco_model_tested,acute_data,final_model)
                ...
                for crossval in crossval_types:
                    for chaco_model_tested in models_tested_ll:
                        for subset in subsets:
                            ...
                            [X, Y, C, lesion_load, site, chronic_ids, ignore1,ignore] = create_data_set(csv_path,site_colname,nemo_path,yvar_colname, subid_colname,chronicity_colname,ensemble_atlas,covariates, verbose, y_var, chaco_type,subset,1,nemo_settings,ll= lesionload_type)
                            print(chronic_ids)
                            if subset == 'acutechronic':
                                # if acutechronic, X (from above) is chronic data.
                                # load the acute data here:
                                [acuteX, acuteY, acuteC, acute_lesion_load, acute_site,acute_ids,ignore2,ignore] = create_data_set(csv_path,site_colname,nemo_path,yvar_colname, subid_colname,chronicity_colname,ensemble_atlas,covariates, verbose, y_var, chaco_type,'acute',1,nemo_settings,ll= lesionload_type)
                                acute_data = {'acute_X':acuteX,'acute_Y':acuteY, 'acute_LL':acute_lesion_load, 'acute_C':acuteC, 'acute_site':acute_site}
                                print(acute_data)
    ...
    print('Number of subsets {}'.format(subsets))
    print(subsets)
    if type(subsets)==int:
        print('sfjsa')
        acutechronic=True
