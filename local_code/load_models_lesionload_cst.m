function [r2scores, correl, nullr2scores, nullcorrel,alphas, feats, allactivations] = load_models_lesionload_cst(varargin)
    ...
    for perm_number=1:24
        r2file = sprintf('%s%i_lesionload_cst_scores.txt', filename_base,perm_number);
        corrfile = sprintf('%s%i_lesionload_cst_correlations.txt', filename_base,perm_number);
...
%         disp(perm_number)
%         alpha = sprintf('%s%i_alphas.txt', filename_base,perm_number-1);
%         feat = sprintf('%s%i_feats.txt', filename_base,perm_number-1);
% 
