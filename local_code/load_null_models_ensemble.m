function [r2scores, correl, nullr2scores, nullcorrel] = load_null_models_ensemble(varargin)
    ...
    for perm_number=1:24
       % r2file = sprintf('%s%i_ensemble_scores_ensemble.txt', filename_base,perm_number-1);
        corrfile = sprintf('%s%i_ensemble_correlations_ensemble.txt', filename_base,perm_number);
