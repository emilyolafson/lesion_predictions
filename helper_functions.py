...
def run_regression_ensemble(X1, C, Y, group, inner_cv_id, outer_cv_id, models_tested, atlas, y_var, chaco_type, subset, save_models,results_path,crossval_type,nperms,null):
    ...
    for n in range(0,nperms):
        ...
        np.save(os.path.join(results_path, filename + "_correlations_lesion.npy"), correlations_lesion)
        np.save(os.path.join(results_path, filename + "_mean_absolute_error.npy"), mean_abs_error)
        np.save(os.path.join(results_path, filename + "_mean_absolute_error_lesion.npy"), mean_abs_error_lesion)
